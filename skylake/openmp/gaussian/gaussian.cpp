 ...
 **-------------------------------------------------------
 */
void Fan1(int gridDimx,int blockDimx, float *m_cuda, float *a_cuda, int Size, int t)
{ 
  omp_set_num_threads(24);
  #pragma omp parallel for schedule(static)  
  for(int idx=0; idx<gridDimx*blockDimx; idx++){
    int blockIdxx=idx/blockDimx;
    int threadIdxx=idx%blockDimx;

	  if(threadIdxx + blockIdxx * blockDimx >= Size-1-t) continue;
	  *(m_cuda+Size*(blockDimx*blockIdxx+threadIdxx+t+1)+t) = *(a_cuda+Size*(blockDimx*blockIdxx+threadIdxx+t+1)+t) / *(a_cuda+Size*t+t);
...
void Fan2(int grid2d, int block2d, float *m_cuda, float *a_cuda, float *b_cuda,int Size, int j1, int t)
{
 omp_set_num_threads(24);
 #pragma omp parallel for schedule(static)    
for(int idx=0; idx<grid2d*grid2d*block2d*block2d; idx++){
  int bidx=idx/(block2d*block2d);
  int tidx=idx%(block2d*block2d);

  int threadIdxx=tidx/block2d;
  int threadIdxy=tidx%block2d;
  int blockIdxx=bidx/grid2d;
  int blockIdxy=bidx%grid2d;

  int blockDimx=block2d;
  int blockDimy=block2d;

  //BEGIN
	if(threadIdxx + blockIdxx * blockDimx >= Size-1-t) continue;
	if(threadIdxy + blockIdxy * blockDimy >= Size-t) continue;
	
	int xidx = blockIdxx * blockDimx + threadIdxx;
	int yidx = blockIdxy * blockDimy + threadIdxy;
...
void ForwardSub()
{
	...
	for (t=0; t<(Size-1); t++) {
		...
		Fan1(grid_size,block_size,m,a,Size,t);
		//Fan2<<<dimGridXY,dimBlockXY>>>(m,a,b,Size,Size-t,t);
		Fan2(gridSize2d,blockSize2d,m,a,b,Size,Size-t,t);
