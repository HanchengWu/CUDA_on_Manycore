...
void run(int argc, char** argv)
{
    ...
    memcpy(MatrixTemp[0], FilesavingTemp, size*sizeof(float) );
    ...
    int ret= compute_tran_temp(FilesavingPower, MatrixTemp,grid_cols,grid_rows, total_iterations,pyramid_height, blockCols, blockRows, borderCols, borderRows);
	printf("Ending simulation\n");
    
    //cudaMemcpy(MatrixOut, MatrixTemp[ret], sizeof(float)*size, cudaMemcpyDeviceToHost);
    //phiMemcpy(MatrixOut, MatrixTemp[ret], size, PhiToCpu);
    memcpy(MatrixOut, MatrixTemp[ret], size*sizeof(float));
