...
void ForwardSub()
{
	...
	//phiMemcpy(m_cuda, m, Size*Size, CpuToPhi);
	//phiMemcpy(a_cuda, a, Size*Size, CpuToPhi);
	//phiMemcpy(b_cuda, b, Size, CpuToPhi);
    ...
    // begin timing kernels
    	...
    	FILE *fresult = fopen("result_skylake.txt","a+");
    	double s_time=gettime_ms();
	
	for (t=0; t<(Size-1); t++) {

		Fan1(ptspc, usage, dimGrid, dimBlock, m, a, Size, t);
		//cudaThreadSynchronize();
		Fan2(ptspc, usage, dimGridXY, dimBlockXY, m, a, b, Size, Size-t, t);
		//cudaThreadSynchronize();
		//checkCUDAError("Fan2");
	}
	double e_time=gettime_ms();

    	fprintf(fresult,"%-10d,%-8d,%-20lf\n",ptspc, Size, e_time-s_time);
...
//	phiMemcpy(m, m_cuda, Size*Size, PhiToCpu);
//	phiMemcpy(a, a_cuda, Size*Size, PhiToCpu);
//	phiMemcpy(b, b_cuda, Size, PhiToCpu);
