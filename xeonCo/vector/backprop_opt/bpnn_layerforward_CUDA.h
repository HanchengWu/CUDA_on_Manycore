...
inline unsigned gfe( __m512i &v) {
        unsigned *tem =(unsigned *)&v;
	return tem[0];
}
...
/*  
	vint tid=threadIdxx;
	vmask mk=_mm512_mask_cmp_epi32_mask(init_mask,tid,vseti(256),_MM_CMPINT_LT);
	while(mk!=0){
	vint tx=_mm512_mask_div_epi32(zero,mk,tid,sixt);
        vint ty=_mm512_mask_rem_epi32(zero,mk,tid,sixt);



	tid=_mm512_mask_add_epi32(tid,mk,tid,sixt);
	mk=_mm512_mask_cmp_epi32_mask(mk,tid,vseti(256),_MM_CMPINT_LT );
	}
 ...
 */
	...
	vint tx=_mm512_mask_div_epi32(zero,mk,tid,sixt);
	 ...
	 _mm512_store_epi32(index_save+gfe(tid), index);
...
//         pthread_barrier_wait(p->barrier); 
	...
	tid=threadIdxx;
	mk=_mm512_mask_cmp_epi32_mask(init_mask,tid,vseti(256),_MM_CMPINT_LT);
	while(mk!=0){
	vint tx=_mm512_mask_div_epi32(zero,mk,tid,sixt);
        vint ty=_mm512_mask_rem_epi32(zero,mk,tid,sixt);

	vint index=_mm512_load_epi32(index_save+gfe(tid));
...
/*MOD_MAN*/
    ...
    if(p[i].warpid==0) {
      ...
      index_save= (int *)_mm_malloc(256*sizeof(int),64);
    }

    p[i].input_node = input_node;
    p[i].weight_matrix = weight_matrix;
    p[i].index_save= index_save;
