...
void Usage(int argc, char**argv){
    fprintf(stderr,"Usage: %s <affinity> <usage> <input_file>\n", argv[0]);
}
...
void BFSGraph( int argc, char** argv) {
  ...
  FILE *fpout = fopen("bfs_times.txt","a+");
  if(!fpout) {
  ...
  double start_time = gettime_ms();
	...
	do
	{
		...
		Kernel( affinity, usage, grid, threads, d_graph_nodes, d_graph_edges, d_graph_mask, d_updating_graph_mask, d_graph_visited, d_cost, no_of_nodes);
		// check if kernel execution generated and error

		Kernel2( affinity, usage, grid, threads, d_graph_mask, d_updating_graph_mask, d_graph_visited, d_over, no_of_nodes);
		// check if kernel execution generated and error
  ...
  fprintf(fpout,"%-10d, %-10lf, %-10d, ", affinity, usage, no_of_nodes);
