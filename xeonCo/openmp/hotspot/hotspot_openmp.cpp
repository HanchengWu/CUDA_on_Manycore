...
void single_iteration(FLOAT *result, FLOAT *temp, FLOAT *power, int row, int col,
					  ...
					  FLOAT step, int num_omp_threads)
{
    FLOAT delta;
    int r, c;
    int chunk;
 ...
 */
void compute_tran_temp(FLOAT *result, int num_iterations, FLOAT *temp, FLOAT *power, int row, int col, int num_omp_threads) 
{
	#ifdef VERBOSE
	int i = 0;
	#endif
...
#pragma offload target(mic: 1) \
        inout(temp:length(array_size) alloc_if(1) free_if(0)  ) \
        in(power: length(array_size) alloc_if(1) free_if(0) ) \
        in(row: alloc_if(1) free_if(1) ) \
        in(col: alloc_if(1) free_if(1) ) \
        in(Cap_1: alloc_if(1) free_if(1) ) \
        in(Rx_1: alloc_if(1) free_if(1) ) \
        in(Ry_1: alloc_if(1) free_if(1) ) \
        in(Rz_1: alloc_if(1) free_if(1) ) \
        in(step: alloc_if(1) free_if(1) ) \
        in(num_iterations: alloc_if(1) free_if(1) ) \
        inout( result: length(array_size) alloc_if(1) free_if(1) ) \
        in(num_omp_threads: alloc_if(1) free_if(1))
        {
            FLOAT* r = result;
            FLOAT* t = temp;
            for (int i = 0; i < num_iterations ; i++)
            {
                ...
                single_iteration(r, t, power, row, col, Cap_1, Rx_1, Ry_1, Rz_1, step, num_omp_threads);
                FLOAT* tmp = t;
                t = r;
                r = tmp;
            }	
...
int main(int argc, char **argv)
{
    ...
    compute_tran_temp(result,sim_time, temp, power, grid_rows, grid_cols, num_omp_threads);
