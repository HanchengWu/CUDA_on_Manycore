...
int main(int argc, char* argv[]) {
	...
	while(!done) {
		...
		char *sandbox_d=(char *)sandbox;

       #pragma offload target(mic:1) \
		inout(z:length(REC_WINDOW) ONCE) \
		inout(target_lat:ONCE) \
		inout(target_long:ONCE) \
		inout(sandbox_d: length(REC_LENGTH * REC_WINDOW) ONCE) \
		inout(rec_count: ONCE)
		{
		int i=0;
		char *rec_iter;
        ...
        for (i = 0; i < rec_count; i++){
			rec_iter = sandbox_d+(i * REC_LENGTH + LATITUDE_POS - 1);
