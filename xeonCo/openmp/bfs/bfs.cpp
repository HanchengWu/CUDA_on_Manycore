...
//	printf("Reading File\n");
  ...
  FILE *fpout = fopen("bfs_omp_nps3_times.txt","a+");
  if(!fpout) {
...
//  		        printf("\nInitialize PHI\n");
	...
	#pragma offload target(mic:1) \
    	in(no_of_nodes: ONCE) \
    	in(num_omp_threads) \
    	in(h_graph_mask: length(no_of_nodes) ONCE)\
    	in(h_graph_nodes: length(no_of_nodes) ONCE)\
    	in(h_graph_edges: length(edge_list_size) ONCE) \
    	in(h_graph_visited: length(no_of_nodes) ONCE) \
    	in(h_updating_graph_mask: length(no_of_nodes) ONCE) \
    	inout(h_cost: length(no_of_nodes) ONCE) \
    	inout(k:ONCE)
	{
