...
int kdtree::set_tree(int dimension, int number){
	DIM=dimension;
	OBN=number;
	in_o=(int *)malloc(sizeof(int)*DIM*OBN);
	treeset=true;
	return 0;
}
...
int kdtree::build_tree(void){
	...
	max_depth=0;
	max_node=0;
	vtxarr=(int *)malloc(OBN*sizeof(int));
	deptharr=(unsigned *)malloc(OBN*sizeof(unsigned));
        edgearr=(int *)malloc(2*OBN*sizeof(int));
	vec=(int *)malloc(sizeof(int)*OBN);
	edge_p=0;
	for (int i=0;i<OBN;++i){
                vec[i]=i;
        }
