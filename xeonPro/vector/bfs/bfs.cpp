...
void BFSGraph( int argc, char** argv) {
  ...
  FILE *fpout = fopen("bfs_nps6_vector_times.txt","a+");
  if(!fpout) {
  ...
  if ( ftell(fpout) == 0 ) { // if the pos is 0, it is empty
    fprintf(fpout,"%-10s, %-10s, ", "Affinity", "# nodes");
  ...
  double start_time = gettime_ms();
	...
	do
	{
		...
		Kernel( affinity, usage, grid, threads, h_graph_nodes, h_graph_edges, h_graph_mask, h_updating_graph_mask, h_graph_visited, h_cost, no_of_nodes);
		// check if kernel execution generated and error

		Kernel2( affinity, usage, grid, threads, h_graph_mask, h_updating_graph_mask, h_graph_visited, &stop, no_of_nodes);
		// check if kernel execution generated and error
