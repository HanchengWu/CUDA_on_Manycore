...
//***/
//void bpnn_create(BPNN *newnet, int n_in, int n_hidden, int n_out) {
//
// // BPNN *newnet;
//
...
void bpnn_layerforward_d(float *l1, float *l2, float **conn, int n1, int n2) {
  float sum;
  int j, k;
...
void bpnn_adjust_weights_d(float *delta, int ndelta, float *ly, int nly, float **w, float **oldw) {
  float new_dw;
  int k, j;
  ly[0] = 1.0;
  //eta = 0.3;
...
void bpnn_train_kernel(BPNN *net, float *eo, float *eh)
{
  ...
  //FILE *fpout = fopen("backprop_skylake_omp_times.txt","a+");
  FILE *fpout = fopen("backprop_omp_nps6_times.txt","a+");
  if(!fpout) {
  ...
  if ( ftell(fpout) == 0 ) { // if the pos is 0, it is empty
    fprintf(fpout,"%-10s, %-10s, ", "#threads", "#elements");
  ...
  bpnn_layerforward_d(net->hidden_units, net->output_units, net->hidden_weights, hid, out);
  ...
  bpnn_adjust_weights_d(net->hidden_delta, hid, net->input_units, in, net->input_weights, net->input_prev_weights);

  end_time = gettime_ms();
  total_time+=end_time-start_time;

  fprintf(fpout,"%-10d, %-10d, ", NUM_THREAD, layer_size);
...
void backprop_face() {
  ...
  int i;
  float out_err, hid_err;
  //bpnn_create(net, layer_size, 16, 1); // (16, 1 can not be changed)
